"""
Stationary_PSS_sampling for TSP
"""
...
class TSP:
    ...
    def load_parameters(self):
        """Load parameter settings based on the instance name, including the penalty factor K, the initial temperature for exponential annealing T0, and the exponential factor alpha"""
        parameter_mapping = {
            "gr17": {"K": 500, "T0": 100, "alpha": 0.976}, "gr24": {"K": 150, "T0": 100, "alpha": 0.955},
            "fri26": {"K": 300, "T0": 80, "alpha": 0.968}, "swiss42": {"K": 150, "T0": 50, "alpha": 0.968},
            "gr48": {"K": 600, "T0": 200, "alpha": 0.972}, "att48": {"K": 2000, "T0": 1000, "alpha": 0.963},
            "hk48": {"K": 3000, "T0": 1000, "alpha": 0.968},  "berlin52": {"K": 2000, "T0": 800, "alpha": 0.966},
            "st70": {"K": 70, "T0": 50, "alpha": 0.955}, "eil76": {"K": 100, "T0": 50, "alpha": 0.956},
            "rd100": {"K": 300, "T0": 300, "alpha": 0.964}, "kroA100": {"K": 2000, "T0": 2000, "alpha": 0.955},
            "kroB100": {"K": 2000, "T0": 2000, "alpha": 0.955}, "kroC100": {"K": 2000, "T0": 2000, "alpha": 0.955},
            "pr107": {"K": 3000, "T0": 4000, "alpha": 0.958}, "gr120": {"K": 300, "T0": 300, "alpha": 0.964},
            "ch130": {"K": 200, "T0": 200, "alpha": 0.972}, "ch150": {"K": 200, "T0": 200, "alpha": 0.972},
            "kroA150": {"K": 2000, "T0": 2000, "alpha": 0.955}, "kroA200": {"K": 2000, "T0": 1000, "alpha": 0.962}
        ...
        print(f"Parameters loaded for {self.instance_name}: penalty_factor: K={self.K}, initial_temperature: T0={self.T0}, exponential_annealing_factor: alpha={self.alpha} (adjustable)")
    ...
    @staticmethod
    def find_eigenvector_for_eigenvalue_one(A, tol=1e-5, tol_1=0.5):
        eigenvalues, eigenvectors = np.linalg.eig(A)

        """PSS establishment verification"""
        # At extremely low temperatures, the transition matrix may have multiple eigenvectors with eigenvalue 1. In practical scenarios, the system can degenerate into greedy decision-making. For now, we will ignore this situation.
    ...
    def Stationary_PSS_TSP_solver(self):
        City_nums = self.city_nums
        D_matrix = self.d_matrix
        T0 = self.T0
        alpha = self.alpha
        ...
        for iter_T in range(annealing_T_number):
            ...
            for iter in range(iteration):
                ...
                if E_swapped < E_now:
                    ...
                    """Calculate the stationary PSS probability distribution (eigenvector corresponding to the eigenvalue 1 of the transition matrix)"""
                    transfer_matrix_12_f = self.find_eigenvector_for_eigenvalue_one(transfer_matrix_12)
                    transfer_matrix_34_f = self.find_eigenvector_for_eigenvalue_one(transfer_matrix_34)
                    """Calculate the sampling(PSS) probabilities of two valid solutions to the TSP problem"""
                    p1 = transfer_matrix_12_f[1] * transfer_matrix_34_f[1]  # Probability of swapping cities
                    p2 = transfer_matrix_12_f[2] * transfer_matrix_34_f[2]  # Probability of not swapping cities
                    """
                    # other two grouping ways
                    # way 2
                    p1 = transfer_matrix_12_f[1] * transfer_matrix_34_f[2]    
                    p2 = transfer_matrix_12_f[2] * transfer_matrix_34_f[1]    
                    # way 3
                    p1 = transfer_matrix_12_f[0] * transfer_matrix_34_f[3]    
                    p2 = transfer_matrix_12_f[3] * transfer_matrix_34_f[0]  
                    """
        ...
        df.to_csv(f"{self.file_path_output_energy_list}/Stationary_PSS_Energy_list_{self.instance_name}.csv", mode='a', index=False, header=True)
        print('The system energy evolution has been exported to an external file')


def main():
    start_time = time.time()
    print('*** Stationary_PSS_TSP_solver ***')
    ...
    tsp.Stationary_PSS_TSP_solver()  # run the main code
    end_time = time.time()
